...
//            }
     ...
     * @author yoko
     * @date 2019/11/25 22:58
     * local:http://localhost:8086/fine/collAc/getGroupNameByMoney
     * 请求的属性类:RequestDidCollectionAccount
     * 必填字段:{"agtVer":1,"clientVer":1,"clientType":1,"ctime":201911071802959,"cctime":201911071802959,"sign":"abcdefg","token":"111111"}
     * 加密字段:{"jsonData":"eyJhZ3RWZXIiOjEsImNsaWVudFZlciI6MSwiY2xpZW50VHlwZSI6MSwiY3RpbWUiOjIwMTkxMTA3MTgwMjk1OSwiY2N0aW1lIjoyMDE5MTEwNzE4MDI5NTksInNpZ24iOiJhYmNkZWZnIiwidG9rZW4iOiIxMTExMTEifQ=="}
     * 客户端加密字段:ctime+cctime+秘钥=sign
     ...
     * }
     */
    @RequestMapping(value = "/getGroupNameByMoney", method = {RequestMethod.POST})
    public JsonResult<Object> getGroupNameByMoney(HttpServletRequest request, HttpServletResponse response, @RequestBody RequestEncryptionJson requestData) throws Exception {
        String sgid = ComponentUtil.redisIdService.getNewId();
        String cgid = "";
        String token = "";
        String ip = StringUtil.getIpAddress(request);
...
//            }
            ...
            // 查询策略里面的每个群创建微信群的最低保底金额
            String groupMinMoney = "";
            StrategyModel strategyGroupMinMoneyQuery = HodgepodgeMethod.assembleStrategyQuery(ServerConstant.StrategyEnum.GROUP_MIN_MONEY.getStgType());
            StrategyModel strategyGroupMinMoneyModel = ComponentUtil.strategyService.getStrategyModel(strategyGroupMinMoneyQuery, ServerConstant.PUBLIC_CONSTANT.SIZE_VALUE_ZERO);
            groupMinMoney = strategyGroupMinMoneyModel.getStgValue();

            // 获取用户信息
            DidModel didQuery = HodgepodgeMethod.assembleDidQueryByDid(did);
            DidModel didModel = (DidModel) ComponentUtil.didService.findByObject(didQuery);
            boolean flag_add = HodgepodgeMethod.checkAddGroup(didModel.getBalance(), groupMinMoney);// 是否有资格创建新群
            ...
            int countAddNum = 0;
            if (flag_add){
                countAddNum = HodgepodgeMethod.getGroupNumByMoney(didModel.getBalance(), groupMinMoney, haveGroupNum);
            }else {
                // 没有资格创建群
                throw new ServiceException(ErrorCode.ENUM_ERROR.DC00046.geteCode(), ErrorCode.ENUM_ERROR.DC00046.geteDesc());
            }
            if (countAddNum <= 0){
                // 余额可支撑的有效群已到达上限
                throw new ServiceException(ErrorCode.ENUM_ERROR.DC00047.geteCode(), ErrorCode.ENUM_ERROR.DC00047.geteDesc());
            }


            // 查询此用户已经有的有效群，查询此用户是否有资格建群
            ...
            Map<String, String> map = ExceptionMethod.getException(e, ServerConstant.PUBLIC_CONSTANT.SIZE_VALUE_TWO);
            log.error(String.format("this DidCollectionAccountController.getGroupNameByMoney() is error , the cgid=%s and sgid=%s and all data=%s!", cgid, sgid, data));
            if (!StringUtils.isBlank(map.get("dbCode"))){
                log.error(String.format("this DidCollectionAccountController.getGroupNameByMoney() is error codeInfo, the dbCode=%s and dbMessage=%s !", map.get("dbCode"), map.get("dbMessage")));
            }
            e.printStackTrace();
            return JsonResult.failedResult(map.get("message"), map.get("code"), cgid, sgid);
        }
