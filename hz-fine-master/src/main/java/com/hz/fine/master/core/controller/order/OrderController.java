...
//            List<DidModel> didList = ComponentUtil.didService.getEffectiveDidByWxGroupList(didQuery);
    ...
    /**
     * @Description: 获取最近的一条派单数据-详情
     * @param request
     * @param response
     * @return com.gd.chain.common.utils.JsonResult<java.lang.Object>
     * @author yoko
     * @date 2019/11/25 22:58
     * local:http://localhost:8086/fine/order/getNewestOrderData
     * 请求的属性类:RequestOrder
     * 必填字段:{"agtVer":1,"clientVer":1,"clientType":1,"ctime":201911071802959,"cctime":201911071802959,"sign":"abcdefg","token":"111111"}
     * 加密字段:{"jsonData":"eyJhZ3RWZXIiOjEsImNsaWVudFZlciI6MSwiY2xpZW50VHlwZSI6MSwiY3RpbWUiOjIwMTkxMTA3MTgwMjk1OSwiY2N0aW1lIjoyMDE5MTEwNzE4MDI5NTksInNpZ24iOiJhYmNkZWZnIiwidG9rZW4iOiIxMTExMTEifQ=="}
     * 客户端加密字段:id+ctime+cctime+秘钥=sign
     ...
     *     "message": "success",
     *     "data": {
     *         "jsonData": "eyJvcmRlck5ld2VzdCI6eyJpc0hhdmUiOjIsIm9yaWdpbiI6Iuivt+iAkOW/g+etieW+hSIsInB1cnBvc2UiOiLor7fnrYnlvoXmlK/ku5jnlKjmiLfov5vnvqQifSwic2lnbiI6IjFlNmU1NDg4MmI1ZDgxMDc4M2M2MGUzOTcwNzk4ZjNhIiwic3RpbWUiOjE1OTUzMjQwMDc1NTZ9"
     *     },
     *     "sgid": "202007211733270000001",
     *     "cgid": ""
     * }
     */
    @RequestMapping(value = "/getNewestOrderData", method = {RequestMethod.POST})
    public JsonResult<Object> getNewestOrderData(HttpServletRequest request, HttpServletResponse response, @RequestBody RequestEncryptionJson requestData) throws Exception{
        String sgid = ComponentUtil.redisIdService.getNewId();
        String cgid = "";
        String token;
        String ip = StringUtil.getIpAddress(request);
...
//            }
            ...
            long stime = System.currentTimeMillis();
            String sign = SignUtil.getSgin(stime, secretKeySign); // stime+秘钥=sign
            String strData = HodgepodgeMethod.assembleNewestOrderDataResult(stime, sign, orderData);
            // 数据加密
            String encryptionData = StringUtil.mergeCodeBase64(strData);
            ResponseEncryptionJson resultDataModel = new ResponseEncryptionJson();
            resultDataModel.jsonData = encryptionData;
            ...
            Map<String,String> map = ExceptionMethod.getException(e, ServerConstant.PUBLIC_CONSTANT.SIZE_VALUE_TWO);
            // 添加异常
            log.error(String.format("this OrderController.getNewestOrderData() is error , the cgid=%s and sgid=%s and all data=%s!", cgid, sgid, data));
            if (!StringUtils.isBlank(map.get("dbCode"))){
                log.error(String.format("this OrderController.getNewestOrderData() is error codeInfo, the dbCode=%s and dbMessage=%s !", map.get("dbCode"), map.get("dbMessage")));
            }
            e.printStackTrace();
            return JsonResult.failedResult(map.get("message"), map.get("code"), cgid, sgid);
        }
...
//            List<DidModel> didList = ComponentUtil.didService.getDidByWxGroupList(didQuery);
//            // check校验是否有有效的用户
//            HodgepodgeMethod.checkEffectiveDidData(didList);
//
//            // 循环筛选有效
...
//    }
    ...
    public JsonResult<Object> groupQrCode(HttpServletRequest request, HttpServletResponse response, @RequestBody RequestEncryptionJson requestData) throws Exception{
        ...
        try{
            ...
            // 循环筛选有效
            DidModel didModel = ComponentUtil.orderService.screenCollectionAccountByPoolTwo(allDidList, requestModel.money);
            // check校验
            HodgepodgeMethod.checkDidAndByAddWxGroupCollectionAccountOrder(didModel);
            ...
            // 存储redis：抢单池的具体某个给出的ID
            String strKeyCache_poolId = CachedKeyUtils.getCacheKey(CacheKey.QR_CODE_POOL_OPEN_ID);
            ComponentUtil.redisService.set(strKeyCache_poolId, String.valueOf(didModel.getPoolOpenId()));
